...
def main(_config, seed):
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            pc_rotated = rotate_back(pcl, R, T)
            ...
            if not _config['use_reflectance']:
                projected_points = depth_img.unsqueeze(0)
            else:
                projected_points = torch.stack((depth_img, refl_img))
        ...
        # Run model
        # if sample['sub_dir'][0] == 'de6c96c4-f2b2-3f0f-9971-ed35f4118c1e':
        #     continue
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                # Run the i-th network
                T_predicted, R_predicted, w_x, w_q = models[iteration](rgb, lidar) # ,t0,r0
                ...
                uv2, depth2, px, py, refl = camera_model.project_pytorch(rotated_point_cloud, real_shape, reflectance)
                uv2 = uv2.t().int()
                ...
                out_cuda2 = F.pad(depth_img2, shape_pad)
                ...
                if T_composed.norm().item() > 3:
                    failure_cnts[iteration+1] = failure_cnts[iteration+1] + 1
                    break
                if T_composed.norm().item() > 3:
                    print(sample['sub_dir'][0])
    ...
    for i in range(len(weights) + 1):
        ...
        print('failure rates:',failure_cnts[i]/(len(errors_t[i])+failure_cnts[i]))
