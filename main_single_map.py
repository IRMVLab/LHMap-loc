...
from models.CMRNet.CMRNet_single_mapping import CMRNet
...
# CCN training
@ex.capture
def train(model, optimizer, u, v, rgb_img, refl_img, flagt, depth, target_transl, target_rot, loss_fn, point_clouds, loss):
    model.train()
...
# CNN test
@ex.capture
def test(model, u, v, rgb_img, refl_img, flagt, depth, target_transl, target_rot, loss_fn, camera_model, point_clouds, loss):
    model.eval()
...
def main(_config, _run, seed):
    ...
    dataset = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'],
                            ...
                            test_sequence='e03')#_config['test_sequence']
    ...
    print('train:', len(TrainImgLoader))
    print('test:', len(TestImgLoader))
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            for idx in range(len(sample['rgb'])):
                ...
                ## utmp[idx] =
                # print('after pad rgb:{}'.format(rgb.shape)) # 3 * 384 * 1280
                # print('after pad lidar:{}'.format(depth_img_no_occlusion.shape)) # 1 * 384 * 1280

                # occlusion_time += (time.time() - occlusion_start_time)
            ...
            loss = train(model, optimizer, ltolu_input, ltolv_input, rgb_input, lidar_input, flagt_input, depth_input,sample['tr_error'],
        ...
        # pose = np.loadtxt('./RT/poseAfter.txt')
        # pose = pose.reshape((pose.shape[0],3,4))
        if (epoch % 10 == 0) or (epoch > 185):
            ## Test ##
            total_test_loss = 0.
            total_test_t = 0.
            total_test_r = 0.
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                loss, trasl_e, rot_e = test(model, ltolu_input, ltolv_input, rgb_input, lidar_input, flagt_input, depth_input, sample['tr_error'],
            ...
            if val_loss < BEST_VAL_LOSS:
                ...
                savefilename = f'{_config["savemodel"]}/checkpoint_r{_config["max_r"]:.2f}_t{_config["max_t"]:.2f}_e{epoch}_{val_loss:.3f}_MappingSJTU.tar'
                torch.save({
                    'config': _config,
                    'epoch': epoch,
